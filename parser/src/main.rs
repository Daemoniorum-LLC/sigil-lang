...
fn main() -> ExitCode {
    ...
    if args.len() < 2 {
        ...
        eprintln!("  check <path>    Check a file and every module it declares, or a directory (--format=json for AI)");
    ...
    match args[1].as_str() {
        ...
        "check" => {
            if args.len() < 3 {
                ...
                eprintln!("Usage: sigil check <file.sg|directory> [--format=json|compact] [--quiet] [--strict] [--no-resolve-calls] [--no-traverse] [--apply-suggestions]");
                return ExitCode::from(1);
            }
            // Parse format option
...
fn check_file(
    ...
    if !target.is_dir() {
        return check_single_file(path, format, quiet, apply_fixes, strict, opts);
    }
    ...
    for file in &files {
        ...
        let result = check_single_file(&file_path, format, quiet, apply_fixes, strict, opts);
...
fn check_single_file(
    ...
    // A `scroll foo;` with no `foo.sg` beside it: the interpreter shrugs and
    ...
    // (`invoke tome·foo` naming a module that does not resolve is a different
    // construct, and `TypeChecker::check_use_tree` errors on it — see #172.)
    ...
    for file in &graph.files {
        ...
        match &file.parsed {
            Ok(ast) => {
                ...
                // Each file resolves its own `invoke tome·` imports against its
                // own directory, not the entry's — a module in a subdirectory
                // has different siblings than the file that declared it.
...
mod build_subsystem_tests {
    use super::{
        build_workspace, check_single_file, collect_check_files, find_manifest_path,
        resolve_source_file, CheckOptions, Manifest, OutputFormat,
    };
    use std::fs;
    use std::path::{Path, PathBuf};
    use std::process::ExitCode;
