...
impl TypeChecker {
    ...
    ///
    /// The builtin table this consults is the interpreter's real one:
    /// `register_builtins` folds every name `register_stdlib` registers into
    /// `stdlib_functions`, so a builtin the interpreter has is one this check
    /// knows. That already had to be true for `--strict`; it matters more here,
    /// because an unknown name is now an error by default.
