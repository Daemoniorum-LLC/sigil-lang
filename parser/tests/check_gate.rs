...
//! ```
//!
//! and a crate whose entry declared `☉ scroll registry;` over a `registry.sg`
//! with a deliberate syntax error in it — `check registry.sg` rc=1,
//! `check lib.sg` rc=0, because the entry check never opened the file.
//!
...
fn a_call_into_a_sibling_module_resolves() {
    ...
    c.file("helpers.sg", "☉ rite helper(x: i64) -> i64 { ⤺ x + 1; }\n");
    let lib = c.file(
        "lib.sg",
...
fn a_syntax_error_behind_a_scroll_fails_the_entry_check() {
    ...
    assert!(
        combined(&entry).contains("registry.sg"),
...
fn invoke_tome_is_traversed_too() {
    ...
    c.file("registry.sg", "rite broken( {\n    ≔ x = 1;\n}\n");
    let lib = c.file(
        "lib.sg",
...
/// A module directory — `registry/mod.sg` — is the other spelling.
#[test]
fn a_module_directory_is_traversed() {
    let c = Crate::new("module-dir");
    c.file("registry/mod.sg", "rite broken( {\n    ≔ x = 1;\n}\n");
    let lib = c.file(
        "lib.sg",
        "☉ scroll registry;\n\nrite main() { print(\"hi\"); }\n",
    );
...
/// `.sg` is canonical and `.sigil` is deprecated, but the interpreter still
/// loads either, trying `.sigil` first. The check has to agree with it about
/// which file a module is, or the two disagree about what the crate contains —
/// which is the whole complaint in LARES-453.
#[test]
fn a_module_resolves_under_either_extension() {
    for ext in ["sigil", "sg"] {
        let c = Crate::new(&format!("module-ext-{}", ext));
        c.file(
            &format!("registry.{}", ext),
            "rite broken( {\n    ≔ x = 1;\n}\n",
        );
        ...
        assert!(
            !out.status.success(),
            "a module stored as .{} went unchecked:\n{}",
            ext,
            combined(&out)
        );
    }
...
fn traversal_reaches_a_module_of_a_module() {
    ...
    c.file("deep.sg", "rite broken( {\n    ≔ x = 1;\n}\n");
    c.file("mid.sg", "☉ scroll deep;\n");
    let lib = c.file(
        "lib.sg",
        "☉ scroll mid;\n\nrite main() { print(\"hi\"); }\n",
    );
...
fn a_module_cycle_terminates() {
    ...
    c.file("a.sg", "☉ scroll b;\n");
    c.file("b.sg", "☉ scroll a;\n");
    let lib = c.file("lib.sg", "☉ scroll a;\n\nrite main() { print(\"hi\"); }\n");
...
fn json_output_attributes_each_diagnostic_to_its_file() {
    ...
    assert!(
        diagnostics
            .iter()
            .any(|d| d["file"].as_str().unwrap_or("").ends_with("registry.sg")),
