...
impl<'a> Parser<'a> {
    ...
    /// Parse a morpheme body: either `{param => expr}` (closure) or `{expr}` (simple expression)
    /// Uses condition context so < is parsed as comparison, not generics
    fn parse_morpheme_body(&mut self) -> ParseResult<Expr> {
        self.expect(Token::LBrace)?;
        self.skip_comments();

        // Check for closure pattern: `{param => expr}` using lookahead
        let is_closure = matches!(self.current_token(), Some(Token::Ident(_)))
            && matches!(self.peek_next(), Some(Token::FatArrow));

        // Parse in condition context so < is comparison, not generics
        ...
        let result = if is_closure {
            ...
            // Simple expression (uses implicit `it` variable)
            let body = self.parse_expr()?;
            self.expect(Token::RBrace)?;
            Ok(body)
        };
    ...
    fn parse_pipe_op(&mut self) -> ParseResult<PipeOp> {
        match self.current_token() {
            Some(Token::Tau) => {
                self.advance();
                let body = self.parse_morpheme_body()?;
            ...
            Some(Token::Rho) => {
                self.advance();
                // Check for operator shorthand: ρ+ or ρ*
                if self.check(&Token::Plus) || self.check(&Token::Star) {
                    let op = if self.consume_if(&Token::Plus) {
                        BinOp::Add
                    } else {
                        self.advance(); // consume Star
                        BinOp::Mul
                    };
                    // Create a synthetic expression for the operator
