...
| **#74** | `main` → `develop`, 97 conflict hunks resolved | ready, green |
| #62 | Lares/Qliphoth statement transform, 96 commits, 875 files | **blocked on #74** |
| ~~#66~~ | the parse_type_path fix standalone | closed — #74 carries it |

```
   #74  main→develop  ──┬──►  release-merge develop→main   (fixes nihil on main)
                        │
                        ├──►  #62 merges develop into itself, then resolves
                        │
                        └──►  downstream consumers repin
```

**#74 is the bottleneck.** PR #62 retargeted itself from `main` to `develop` citing #69, and
its author reached the same conclusion independently: resolving its 13 conflicting files now
"would be throwaway work against a `develop` that #74 replaces". Two workstreams are queued.

**The release-merge back to `main` is the half nobody has done.** Until it happens, `main`
...
1. **The release-merge `develop` → `main`.** Nothing else closes the divergence.
