...
/**
 * POST /compile - Compile Sigil to WebAssembly
 * Returns the compiled WASM binary for download
 */
app.post('/compile', async (req, res) => {
  try {
    const { code, filename = 'program' } = req.body;
    ...
    const safeFilename = filename.replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 32) || 'program';

    const tempFile = join(TEMP_DIR, `${randomUUID()}.sigil`);
    const wasmFile = join(TEMP_DIR, `${randomUUID()}.wasm`);
    ...
    try {
        ...
        execFile(
          ...
          async (error, stdout, stderr) => {
            ...
            if (error) {
              ...
              unlink(wasmFile).catch(() => {});
              resolve({
                success: false,
                error: {
                  type: error.killed ? 'timeout' : 'compile_error',
                  message: stderr || error.message,
                },
                execution_time_ms: executionTimeMs,
              });
            ...
            try {
              ...
              resolve({
                ...
                error: {
                  type: 'internal',
                  message: 'Failed to read compiled WASM',
                },
                execution_time_ms: executionTimeMs,
              });
            }
      ...
      if (result.success) {
        // Send as binary with proper headers for download
        res.setHeader('Content-Type', 'application/wasm');
        res.setHeader('Content-Disposition', `attachment; filename="${safeFilename}.wasm"`);
    ...
    console.error(`[${req.id}] Compile error:`, e);
    res.status(500).json({
      success: false,
      error: {
        type: 'internal',
